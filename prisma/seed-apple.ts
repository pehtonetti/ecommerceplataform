...
async function main() {
    ...
    for (const p of products) {
        // Cast 'data' as any to bypass potential type mismatch if client isn't regenerated yet
        // But since we are creating, we should check if product exists first to avoid duplicates or upsert

        // Using upsert by name (if unique) or just create. Name is typically unique in this seed context.
        ...
        // Add images
        await prisma.productImage.deleteMany({ where: { productId } }); // Clean old images
